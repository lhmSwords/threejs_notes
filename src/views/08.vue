...
function init() {
    ...
    gui.add(cube.position, 'x').min(0).max(5).step(0.01).name("移动X轴").onChange((value: any) => {
    ...
    const params = {
        ...
        animateRestart: () => {
            animateList.forEach((animate: gsap.core.Tween) => animate.restart())
        },
        animatePause: () => {
            animateList.forEach((animate: gsap.core.Tween) => animate.pause())
        },
        animateStop: () => {
            animateList.forEach((animate: gsap.core.Tween) => animate.kill())
        },
        animateResume: () => {
            animateList.forEach((animate: gsap.core.Tween) => animate.resume())
        },
    };
    gui.addColor(params, 'color').onChange((value: string) => {
        cube.material.color.set(value);
    });
    gui.add(params, "fullScreen").name("全屏");
    const animateFolder = gui.addFolder("运动控制");
    animateFolder.open();
    animateFolder.add(params, "animateRestart").name("动画重播");
    animateFolder.add(params, "animatePause").name("动画暂停");
    animateFolder.add(params, "animateStop").name("动画关闭");
    animateFolder.add(params, "animateResume").name("动画恢复");

    const setCubeFolder = gui.addFolder("设置立方体");
    setCubeFolder.open();
    setCubeFolder.add(cube.material, "wireframe").name("线框几何体");
